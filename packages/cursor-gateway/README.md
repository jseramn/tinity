...
Harnesses MUST preflight GET /health then GET /v1/models before POST (inspectHealth / inspectModelsList / preflightWrap, or package script preflight): require workspace+model with explicit fast=false, refuse busy (retry_after: 1), stale health, models 404, workspace mismatch, or Fast. HTTP timeout 2000ms. Ok forwards jobTimeoutMs and fast when present. Do not send Casos work to a wrap bound to another tree.
...
- preflight refuses stale health, models 404, busy, workspace mismatch, Fast, model without fast=false, health.fast true, or invalid jobTimeoutMs; busy includes retry_after: 1; missing jobTimeoutMs or fast is live-old ok; when present, jobTimeoutMs and fast are forwarded on ok
